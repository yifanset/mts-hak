...
self.addEventListener('message', function(e) {
  ...
  if (!e.data || typeof e.data !== 'object') {
    // Silently ignore ping or health check messages
    return;
  }
  
  const { action, data } = e.data;
  
  // Handle empty or undefined action more gracefully
  if (!action) {
    // Likely just a ping from the main thread to check if worker is alive
    return;
  }
